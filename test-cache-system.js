...
const { cacheInvalidator, CacheInvalidation } = require('./lib/cache-invalidation');
...
async function testCacheSystem() {
  ...
  try {
    ...
    await cache.set('test:key1', { message: 'Hello Cache!', timestamp: Date.now() }, {
      ttl: 60,
      tags: ['test']
    });
    ...
    await patientCache.set('patient:123', {
      id: '123',
      name: 'Test Patient',
    ...
    console.log('✅ Session destruction:', !destroyedSession ? 'PASSED' : 'FAILED');
...
async function performanceTest() {
  ...
  for (let i = 0; i < iterations; i++) {
    await cache.set(`perf:test:${i}`, { value: i, timestamp: Date.now() }, {
      ttl: 300,
      layer: 'both'
    });
  }
  ...
  console.log(`⏱️ Cache SET performance: ${setTime}ms for ${iterations} operations`);
  console.log(`   Average: ${(setTime / iterations).toFixed(2)}ms per operation`);
  ...
  console.log(`⏱️ Cache GET performance: ${retrieveTime}ms for ${iterations} operations`);
  console.log(`   Average: ${(retrieveTime / iterations).toFixed(2)}ms per operation`);
