...
const DEFAULT_SETTINGS: SchedulingSettings = {
  limits: {
    ...
    saturday: [{ id: 'sat1', startTime: '07:00', endTime: '13:00', limit: 3 }],
  },
...
class SchedulingSettingsService {
  getSettings(): SchedulingSettings {
    try {
      ...
      console.error(
        'Failed to parse scheduling settings from localStorage',
        error
      );
    }
  ...
  saveSettings(settings: SchedulingSettings): void {
    try {
      ...
      settings.limits.weekday.sort((a, b) =>
        a.startTime.localeCompare(b.startTime)
      );
      settings.limits.saturday.sort((a, b) =>
        a.startTime.localeCompare(b.startTime)
      );
      localStorage.setItem(SETTINGS_KEY, JSON.stringify(settings));
    } catch (error) {
      console.error(
        'Failed to save scheduling settings to localStorage',
        error
      );
    }
  }
  ...
  getSlotOccupancy(
    ...
    const rules = isSaturday
      ? settings.limits.saturday
      : settings.limits.weekday;
    const rule = rules.find(r => time >= r.startTime && time < r.endTime) || {
      limit: 0,
    };
    ...
    const evaluationsInSlot = appointmentsInSlot.filter(
      app => app.type === AppointmentType.Evaluation
    );

    const isPatientLimitFull = appointmentsInSlot.length >= rule.limit;
    const isEvalLimitFull =
      evaluationsInSlot.length >= settings.maxEvaluationsPerSlot;
