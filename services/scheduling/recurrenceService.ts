 ...
 */
export const generateRecurrences = (
  ...
  const duration =
    initialAppointment.endTime.getTime() -
    initialAppointment.startTime.getTime();
  ...
  while (currentDate <= untilDate) {
    if (recurrenceRule.days.includes(currentDate.getDay())) {
      ...
      if (
        currentDate.getTime() >=
        new Date(initialAppointment.startTime).setHours(0, 0, 0, 0)
      ) {
        const startTime = new Date(currentDate);
        startTime.setHours(
          initialAppointment.startTime.getHours(),
          initialAppointment.startTime.getMinutes()
        );
