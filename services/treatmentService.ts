...
export const getPlanByPatientId = async (
  patientId: string
...
export const getExercisesByPlanId = async (
  planId: string
): Promise<ExercisePrescription[]> => {
  await delay(300);
  return db
    .getExercisePrescriptions()
    .filter(ex => ex.treatmentPlanId === planId);
};
...
export const updatePlan = async (
  ...
  if (index > -1) {
    ...
    const newExercises: ExercisePrescription[] = (updates.exercises || []).map(
      (ex, i) => ({
        id: `ex_${Date.now()}_${i}`,
    ...
    const updatedPlan: TreatmentPlan = {
      ...
      exercises:
        newExercises.length > 0 ? newExercises : existingPlan.exercises,
    };
