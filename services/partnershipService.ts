...
export const savePartner = async (
  partnerData: Omit<Partner, 'id' | 'avatarUrl'> & { id?: string }
): Promise<Partner> => {
  await delay(400);
  if (partnerData.id) {
...
export const sellVoucher = async (
  patientId: string,
  partnerId: string,
  planId: string
): Promise<Voucher> => {
...
export const getEducatorClients = async (
  educatorId: string
): Promise<PartnershipClient[]> => {
  await delay(500);
...
export const getIntegratedClientHistory = async (
  clientId: string,
  educatorId: string
): Promise<IntegratedClientHistory | null> => {
  await delay(700);
