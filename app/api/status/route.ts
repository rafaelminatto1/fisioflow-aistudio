...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const cpuPercentage = Math.round((cpuUsage.user + cpuUsage.system) / 1000000) // Convert to percentage approximation
    ...
    const allServicesOperational = Object.values(services).every(service => service.status === 'operational')
    ...
    const response: SystemStatusResponse = {
      ...
      metrics: {
        ...
        cpu: {
          ...
          ...(isAuthenticated && process.platform !== 'win32' && {
      ...
      deployment: {
        ...
        ...(isAuthenticated && {
          ...
          branch: process.env.VERCEL_GIT_COMMIT_REF || process.env.RAILWAY_GIT_BRANCH || 'main'
        })
      }
    }

    // Return appropriate status code based on system status
    const statusCode = systemStatus === 'operational' ? 200 : 
