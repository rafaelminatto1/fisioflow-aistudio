  ...
  const stats: DashboardStats = useMemo(() => {
    ...
    const revenueThisMonth = appointments
      .filter(
        app =>
          ...
          new Date(app.startTime) >= startOfThisMonth
      )
    ...
    const revenueLastMonth = appointments
      .filter(app => {
        ...
        return (
          app.status === AppointmentStatus.Completed &&
          appDate >= startOfLastMonth &&
          appDate < startOfThisMonth
        );
      })
      .reduce((sum, app) => sum + app.value, 0);

    const newPatientsThisMonthCount = patients.filter(
      p => new Date(p.registrationDate) >= startOfThisMonth
    ).length;
    ...
    const newPatientsChange = calculateChange(
      newPatientsThisMonthCount,
      newPatientsLastMonthCount
    );
