...
const MonitoringDashboard: React.FC = () => {
  ...
  const [preventionSystemStatus, setPreventionSystemStatus] = useState<'running' | 'stopped' | 'unknown'>('unknown');
  ...
  if (!systemStatus) {
    return (
      <Alert>
        ...
        <AlertDescription>
          Não foi possível carregar o status do sistema. Verifique se a API está funcionando.
        </AlertDescription>
      </Alert>
    );
  }
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {systemStatus.diagnostics.critical} críticos, {systemStatus.diagnostics.high} altos
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Sistema de Prevenção</CardTitle>
            <Shield className={`h-4 w-4 ${
              preventionSystemStatus === 'running' ? 'text-green-600' : 
              preventionSystemStatus === 'stopped' ? 'text-red-600' : 'text-gray-600'
            }`} />
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div className={`text-lg font-bold ${
                ...
                {preventionSystemStatus === 'running' ? 'ATIVO' : 
                 preventionSystemStatus === 'stopped' ? 'PARADO' : 'DESCONHECIDO'}
              </div>
              <Button
                variant="outline"
                size="sm"
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">{systemStatus.metrics.cpu}%</span>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-muted-foreground">{systemStatus.metrics.memory}%</span>
                  </div>
                  <Progress value={systemStatus.metrics.memory} className="h-2" />
                </div>
                
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium">Disco</span>
                    <span className="text-sm text-muted-foreground">{systemStatus.metrics.disk}%</span>
                  </div>
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  <span className="text-sm text-muted-foreground">{systemStatus.metrics.responseTime}ms</span>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={[
                    { name: 'Críticos', value: systemStatus.diagnostics.critical },
                    { name: 'Altos', value: systemStatus.diagnostics.high },
                    { name: 'Médios', value: systemStatus.diagnostics.medium },
              ...
              <div className="space-y-2">
                {systemStatus.issues.map((issue) => (
                  <div key={issue.id} className={`p-3 border rounded ${
                    issue.resolved ? 'bg-green-50 border-green-200' : 'bg-white'
                  }`}>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        {getSeverityBadge(issue.severity)}
                        <Badge variant="outline">{issue.category}</Badge>
                        <span className={issue.resolved ? 'line-through text-muted-foreground' : ''}>
                          {issue.description}
                        </span>
                      </div>
                      <div className="text-xs text-muted-foreground">
