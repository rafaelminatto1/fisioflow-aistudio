...
const inputStyle =
  'block w-full px-3 py-2 bg-white border border-slate-300 rounded-lg shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 sm:text-sm';
...
const SportsAssessmentForm: React.FC<SportsAssessmentFormProps> = ({
  patientId,
  onBack,
}) => {
  const {
  ...
  } = useForm<SportsAssessmentFormData>({
    resolver: zodResolver(sportsAssessmentSchema),
    defaultValues: {
      patientId,
      functionalTests: [],
      injuryHistory: { painLevel: 0 },
    },
  });
    ...
    >
        ...
        <div className='space-y-6'>
          ...
          <fieldset className={fieldsetStyle}>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-2'>
              <input
                type='text'
                placeholder='Modalidade Esportiva*'
                {...register('sport')}
              ...
              <input
                type='text'
                placeholder='Frequência de treinos/sem'
          ...
          <fieldset className={fieldsetStyle}>
            <legend className={legendStyle}>
              Histórico da Lesão (se aplicável)
            </legend>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-2'>
              <textarea
                placeholder='Quando iniciou e como?'
                {...register('injuryHistory.startDate')}
                className={`${inputStyle} md:col-span-2`}
                rows={2}
              />
              <textarea
                placeholder='Houve algum evento específico associado?'
                {...register('injuryHistory.triggerEvent')}
                className={`${inputStyle} md:col-span-2`}
                rows={2}
              />
              <textarea
                placeholder='Movimentos/Fatores que agravam a dor'
                {...register('injuryHistory.aggravatingFactors')}
                className={`${inputStyle} md:col-span-2`}
                rows={2}
              />
              <div className='md:col-span-2'>
                ...
                <input
                  ...
                  max='10'
                  defaultValue='0'
                  {...register('injuryHistory.painLevel', {
                    valueAsNumber: true,
                  })}
          ...
          <fieldset className={fieldsetStyle}>
            ...
            <div className='space-y-4 mt-2'>
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  className='flex items-center gap-2 animate-fade-in-fast'
                >
                  <input
                    {...register(`functionalTests.${index}.testName`)}
                    placeholder='Nome do Teste'
                    className={`${inputStyle} flex-1`}
                  />
                  <input
                    {...register(`functionalTests.${index}.result`)}
              ...
              >
                <PlusCircle className='mr-2 w-5 h-5' /> Adicionar Teste
                Funcional
              </button>
            </div>
          </fieldset>
      ...
      <div className='flex justify-end p-4 bg-white rounded-2xl shadow-sm'>
        <button
          ...
          className='w-full sm:w-auto inline-flex items-center justify-center bg-teal-500 text-white font-bold py-3 px-8 rounded-lg hover:bg-teal-600 transition-colors disabled:bg-teal-300'
        >
