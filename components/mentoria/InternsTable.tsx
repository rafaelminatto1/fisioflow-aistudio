...
const InternsTable: React.FC<InternsTableProps> = ({
  interns,
  onAdd,
  onEdit,
}) => {
        ...
        <table className='min-w-full divide-y divide-slate-200'>
          <thead className='bg-slate-50'>
            <tr>
              <th
                scope='col'
                className='p-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider'
              >
                Nome
              </th>
              <th
          ...
          <tbody className='bg-white divide-y divide-slate-200'>
            {interns.map(intern => (
              ...
              >
                ...
                <td className='p-3 whitespace-nowrap'>
                  <span
                    className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColorMap[intern.status]}`}
                  >
                    {intern.status}
                  </span>
                </td>
                <td className='p-3 whitespace-nowrap text-sm font-semibold text-slate-600'>
                  {intern.averageGrade?.toFixed(1) || 'N/A'}
                </td>
              </tr>
            ))}
          </tbody>
