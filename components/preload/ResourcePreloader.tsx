...
async function checkResourceExists(url: string): Promise<boolean> {
  try {
    ...
    if (url.includes('/api/') || url.includes('?') || url.includes('#')) {
      console.warn(
        `[ResourcePreloader] Ignorando verificação de API/rota dinâmica: ${url}`
      );
      return false;
    }
    ...
    console.warn(
      `[ResourcePreloader] Erro ao verificar recurso ${url}:`,
      error
    );
    return false;
  }
...
function preloadResource(resource: string): void {
  try {
    ...
    if (resource.endsWith('.css')) {
      ...
      console.warn(
        `[ResourcePreloader] Tipo de recurso não suportado: ${resource}`
      );
      return;
    }
    ...
    link.onerror = () => {
      console.warn(
        `[ResourcePreloader] Falha ao carregar recurso: ${resource}`
      );
    };
    ...
    console.error(
      `[ResourcePreloader] Erro ao precarregar recurso ${resource}:`,
      error
    );
  }
}
        ...
        try {
          ...
          if (resource.startsWith('/')) {
            ...
            if (exists) {
              ...
              console.warn(
                `[ResourcePreloader] Recurso não encontrado, ignorando: ${resource}`
              );
            }
          }
        } catch (error) {
          console.error(
            `[ResourcePreloader] Erro ao verificar recurso ${resource}:`,
            error
          );
        }
      }
