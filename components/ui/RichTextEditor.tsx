...
const RichTextEditor: React.FC<RichTextEditorProps> = ({
  ...
  const applyFormatting = (format: 'bold' | 'italic' | 'list') => {
    ...
    if (format === 'list') {
      ...
      const areAllLinesBulleted = lines.every(line =>
        line.trim().startsWith('* ')
      );

      const newSelectedLines = lines
        .map(line => {
  ...
  return (
    <div className='flex flex-col h-full'>
      <div
        className={`flex items-center gap-2 border border-b-0 border-slate-300 p-2 rounded-t-lg ${disabled ? 'bg-slate-200' : 'bg-slate-50'}`}
      >
        <button
          type='button'
