...
const PatientReport: React.FC<PatientReportProps> = ({ patients }) => {
  const patientStats = useMemo(() => {
    ...
    const newThisMonth = patients.filter(
      p => new Date(p.registrationDate) >= new Date(new Date().setDate(1))
    ).length;
  ...
  const newPatientsByMonth = useMemo(() => {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const monthString = date.toLocaleDateString('pt-BR', {
        month: 'short',
        year: '2-digit',
      });
  ...
  const statCards = [
    ...
    {
      title: 'Inativos/Alta',
      value: (patientStats.Inactive + patientStats.Discharged).toString(),
      icon: <UserX />,
    },
  ];
