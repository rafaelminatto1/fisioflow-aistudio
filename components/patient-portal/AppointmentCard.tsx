...
const AppointmentCard: React.FC<AppointmentCardProps> = ({
  ...
  return (
    <div
      className={`bg-white p-4 rounded-xl shadow-sm border-l-4 ${isPast ? 'border-slate-300' : 'border-teal-500'}`}
    >
      <div className='flex justify-between items-start'>
        <div>
          <p
            className={`font-bold text-slate-800 ${isPast ? 'text-slate-600' : ''}`}
          >
            {appointment.type}
          </p>
        ...
        <div
          className={`flex items-center px-2 py-1 rounded-full text-xs font-semibold ${currentStatus.color}`}
        >
          {currentStatus.icon}
