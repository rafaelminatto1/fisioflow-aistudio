...
const VoucherPurchaseModal: React.FC<VoucherPurchaseModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  plan,
      ...
      >
        ...
        <main className='p-6'>
          <p className='text-slate-600 mb-4'>
            Você está prestes a adquirir o seguinte plano:
          </p>
          ...
          <div className='mt-6 flex items-center text-xs text-slate-500 bg-slate-100 p-2 rounded-md'>
            ...
            <span>
              Este é um ambiente de simulação. Nenhum pagamento real será
              processado. Seus dados estão seguros.
            </span>
          </div>
        </main>
        <footer className='flex justify-end items-center p-4 border-t border-slate-200 bg-slate-50 rounded-b-2xl'>
          ...
          <button
            ...
            className='px-6 py-2 text-sm font-medium text-white bg-teal-500 rounded-lg hover:bg-teal-600 flex items-center disabled:bg-teal-300'
          >
            {isProcessing ? (
