...
const ExerciseEvaluationCard: React.FC<ExerciseEvaluationCardProps> = ({
  exercise,
  prescription,
  todaysEvaluation,
  onSave,
    ...
    <div className='bg-white rounded-2xl shadow-sm flex flex-col overflow-hidden'>
      ...
      <div className='p-4 flex-grow flex flex-col'>
        ...
        <p className='text-sm text-slate-600 mb-2'>
          {prescription.sets} séries de {prescription.repetitions} repetições
        </p>
      ...
      <div className='p-4 bg-slate-50 border-t'>
        ...
        <div className='space-y-4 mt-4'>
          <div>
            <label className='text-sm font-medium text-slate-600 block mb-2 text-center'>
              Dificuldade
            </label>
            <div className='flex justify-around'>
              {ratingOptions.map(opt => (
                <button
                  ...
                  className={`flex flex-col items-center p-2 rounded-lg w-20 transition-all ${rating === opt.value ? 'bg-teal-100 ring-2 ring-teal-500' : 'hover:bg-slate-100'}`}
                >
                  {opt.icon}
          ...
          <button
            onClick={handleSave}
            disabled={
              isSaving || rating === undefined || painLevel === undefined
            }
