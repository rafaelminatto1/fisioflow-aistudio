...
const VoucherPlanCard: React.FC<VoucherPlanCardProps> = ({
  plan,
  onSelect,
}) => {
  return (
    <div
      className={`rounded-2xl border-2 p-6 flex flex-col h-full transition-all duration-300 ${plan.popular ? 'border-teal-500 shadow-lg' : 'border-slate-200 bg-white'}`}
    >
      {plan.popular && (
