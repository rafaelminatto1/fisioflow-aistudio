...
export function SecurityDashboard() {
  ...
  const clearOldLogs = async (days: number = 30) => {
    try {
      const response = await fetch(`/api/security?action=clear_old_logs&days=${days}`, {
        method: 'DELETE'
      });
  ...
  const convertToCSV = (logs: AuditLog[]) => {
    const headers = ['ID', 'Event Type', 'Table', 'Record ID', 'User ID', 'IP Address', 'Created At'];
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{securityData.metrics.activePolicies}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">RLS Habilitado</CardTitle>
            <ShieldCheck className="h-4 w-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{securityData.metrics.rlsEnabled}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{securityData.metrics.failedAttempts}</div>
            <p className="text-xs text-muted-foreground">
              últimas 24h
            </p>
          </CardContent>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Acessos Bem-sucedidos</CardTitle>
            <CheckCircle className="h-4 w-4 text-green-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{securityData.metrics.successfulAccess}</div>
            <p className="text-xs text-muted-foreground">
              últimas 24h
            </p>
          </CardContent>
                ...
                {securityData.rlsStatus.map((table) => (
                  <div key={table.tableName} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant={table.rlsEnabled ? 'default' : 'destructive'}>
                        {table.rlsEnabled ? 'Habilitado' : 'Desabilitado'}
                      </Badge>
                      <Switch
                        checked={table.rlsEnabled}
                        onCheckedChange={(checked) => toggleRLS(table.tableName, checked)}
                      />
                    </div>
                  </div>
                ))}
                  ...
                  <div key={log.id} className="flex items-center justify-between p-3 border rounded text-sm">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {log.tableName && (
                          <span className="text-muted-foreground">em {log.tableName}</span>
                        )}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        IP: {log.ipAddress || 'N/A'} | User: {log.userId || 'N/A'}
                      </p>
                    </div>
