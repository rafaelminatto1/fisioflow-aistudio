...
export default function PatientList({ initialData }: PatientListProps) {
  ...
  //  Implementar a lógica de 'carregar mais' aqui
          ...
          {!isPending &&
            patients.map(patient => (
              ...
              >
                ...
                <span
                  className={`ml-4 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${statusColorMap[patient.status] || 'bg-slate-100 text-slate-800'}`}
                >
                  {patient.status}
                </span>
              </div>
