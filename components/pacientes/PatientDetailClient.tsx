...
const InfoPill: React.FC<{ label: string; value: React.ReactNode }> = ({
  label,
  value,
}) => (
  <div>
          ...
          <div>
            ...
            {patient.treatmentPlan ? (
              <p className='text-sm text-slate-600'>
                {patient.treatmentPlan.treatmentGoals}
              </p>
            ) : (
              <p className='text-sm text-slate-500'>
                Nenhum plano de tratamento ativo.
              </p>
            )}
          </div>
        )}
        {activeTab === 'documentos' && (
          <div>Upload e Lista de Documentos aqui...</div>
        )}
        {activeTab === 'historico' && (
          <div>
            <div className='flex justify-between items-center mb-4'>
              ...
              <button
                ...
                className='inline-flex items-center rounded-lg border border-transparent bg-sky-500 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-sky-600'
              >
