...
const EventDashboardMetrics: React.FC<EventDashboardMetricsProps> = ({
  event,
  registrations,
}) => {
  const { totalRegistered, checkInRate, revenue, checkInCount } =
    useMemo(() => {
    ...
    <div className='bg-slate-50 p-6 rounded-2xl border border-slate-200'>
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        ...
        <div className='md:col-span-2 bg-white p-6 rounded-xl shadow-sm flex flex-col justify-center'>
          <h4 className='font-semibold text-slate-700 text-center mb-3 text-lg'>
            Taxa de Check-in
          </h4>
          <div className='w-full bg-slate-200 rounded-full h-8 relative'>
            <div
              className={`${checkInColor} h-8 rounded-full transition-all duration-500`}
              style={{ width: `${checkInRate}%` }}
            ></div>
            <span className='absolute inset-0 flex items-center justify-center text-sm font-bold text-white drop-shadow-sm'>
              {checkInRateFormatted}
            </span>
          </div>
          <p className='text-center text-xs text-slate-500 mt-2'>
            {checkInCount} de {totalRegistered} participantes compareceram.
          </p>
        </div>
      </div>
    </div>
