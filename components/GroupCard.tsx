    ...
    <div className='bg-white rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 flex flex-col group'>
      <div className='p-5 flex-grow'>
        <div className='flex justify-between items-start'>
          ...
          <button
            onClick={onEdit}
            className='p-2 rounded-full text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-slate-100 hover:text-teal-600'
          >
            <Edit className='w-4 h-4' />
          </button>
        </div>
        <p className={`text-sm font-semibold text-${therapist?.color}-600`}>
          {therapist?.name}
        </p>
        ...
        <div className='mt-4 space-y-2 text-sm'>
          <div className='flex items-center text-slate-600'>
            ...
            <span>
              {group.capacity.current} / {group.capacity.max} Participantes
            </span>
          </div>
          <div className='flex items-center text-slate-600'>
            <Clock className='w-4 h-4 mr-2 text-slate-400' />
            <span>
              {group.schedule.days.join(', ')} às {group.schedule.time}
            </span>
          </div>
          <div className='flex items-center text-slate-600'>
        ...
        <div className='flex -space-x-2 overflow-hidden'>
          {group.members.slice(0, 5).map(member => (
            <img
              ...
              alt={member.patientName}
              title={member.patientName}
            />
          ))}
