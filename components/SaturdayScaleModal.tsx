...
const SaturdayScaleModal: React.FC<SaturdayScaleModalProps> = ({
  isOpen,
  onClose,
  appointments,
}) => {
  const saturdaySchedule = useMemo(() => {
    ...
    const schedule = saturdayAppointments.reduce(
      (acc, app) => {
        const time = app.startTime.toLocaleTimeString('pt-BR', {
          hour: '2-digit',
          minute: '2-digit',
        });
        if (!acc[time]) {
    ...
    const sortedSchedule = Object.fromEntries(
      Object.entries(schedule).sort(([timeA], [timeB]) =>
        ...
        <header className='flex items-center justify-between p-5 border-b border-slate-200'>
          <div>
            <h2 className='text-lg font-bold text-slate-800'>
              Escala do Próximo Sábado
            </h2>
            {saturdaySchedule.date && (
              <p className='text-sm text-slate-500'>
                {saturdaySchedule.date.toLocaleDateString('pt-BR', {
                  weekday: 'long',
                  day: '2-digit',
                  month: 'long',
                })}
          ...
          {scheduleKeys.length > 0 ? (
            <ul className='space-y-4'>
              {scheduleKeys.map(time => (
                <li key={time}>
                  <div className='px-2 py-1 bg-slate-100 rounded-md font-bold text-slate-700 text-sm mb-2'>
                    {time}
                  </div>
                  <ul className='space-y-1 pl-4'>
                    {(saturdaySchedule.schedule as Record<string, string[]>)[
                      time
            ...
            <div className='text-center text-slate-500 py-10'>
              ...
              <p className='mt-1 text-sm text-slate-500'>
                A agenda para o próximo sábado está vazia.
              </p>
            </div>
          )}
        </main>
