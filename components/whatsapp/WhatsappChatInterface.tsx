      ...
      if (text.trim() === '1') {
        ...
        if (plan && plan.exercises) {
          const exerciseList = plan.exercises
            .map(ex => ` - ${ex.exerciseName} (${ex.sets}x${ex.repetitions})`)
            .join('\n');
          addBotResponse(
            `Claro! Seus exercícios para hoje são:\n${exerciseList}\n\nLembre-se de marcar como feito no seu portal! 😉`
          );
        } else {
          addBotResponse(
            'Você não tem um plano de exercícios ativo no momento.'
          );
        }
      } else if (text.trim() === '2') {
        addBotResponse(
          "Para agendar, preciso saber o melhor dia e período para você. Por exemplo: 'terça de manhã' ou 'sexta à tarde'."
        );
        setConversationState('scheduling_day');
      } else if (text.trim() === '3') {
        addBotResponse(
          'Ok! Um de nossos atendentes entrará em contato com você por aqui em breve. Por favor, aguarde.'
        );
        setConversationState('waiting_human');
      } else {
        addBotResponse(
          'Desculpe, não entendi. Por favor, escolha uma das opções (1, 2 ou 3).'
        );
      }
    } else if (conversationState === 'scheduling_day') {
      addBotResponse(
        `Perfeito! Verificando horários para *${text}*... Encontrei uma vaga às 10:00. Deseja confirmar? (Sim/Não)`
      );
      setConversationState('scheduling_confirm');
    } else if (conversationState === 'scheduling_confirm') {
      if (text.toLowerCase().includes('sim')) {
        addBotResponse(
          'Ótimo! Sua consulta foi agendada. Você receberá uma confirmação com todos os detalhes. Até breve!'
        );
      } else {
        addBotResponse(
          'Entendido. Se quiser tentar outra data, é só me dizer.'
        );
      }
      setConversationState('initial');
    } else if (conversationState === 'waiting_human') {
      addBotResponse(
        'Nossos atendentes ainda estão ocupados. Assim que um estiver disponível, ele responderá aqui. Obrigado pela paciência!'
      );
    }
  };
      ...
      <div className='w-full max-w-lg mx-auto bg-white border border-slate-200 rounded-2xl shadow-lg flex flex-col'>
        ...
        <div className='p-3 bg-slate-100 rounded-t-2xl flex items-center border-b'>
          <div className='relative'>
            <Image
              src='https://i.pravatar.cc/150?u=clinic_logo'
              alt='FisioFlow'
              width={40}
              height={40}
          ...
          <div className='space-y-4'>
            {messages.map(msg => (
              ...
              >
                ...
                <div
                  className={`max-w-xs md:max-w-sm px-4 py-2 rounded-2xl ${msg.sender === 'user' ? 'bg-teal-500 text-white rounded-br-none' : 'bg-white text-slate-800 rounded-bl-none shadow-sm'}`}
                >
        ...
        <div className='p-3 bg-slate-100 rounded-b-2xl border-t'>
          <div className='flex items-center space-x-2'>
            <input
              ...
              onKeyPress={e =>
                e.key === 'Enter' && input.trim() && handleUserInput(input)
              }
