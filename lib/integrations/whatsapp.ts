...
export class WhatsAppIntegration {
  ...
  private async makeRequest(endpoint: string, data: any): Promise<WhatsAppResponse> {
    ...
    return trackExternalAPICall('whatsapp', endpoint, async () => {
      try {
        const response = await fetch(`${this.baseUrl}/${this.version}/${this.phoneNumberId}/${endpoint}`, {
          method: 'POST',
          headers: {
  ...
  async sendAppointmentReminder(
    ...
    if (!patient.phone || patient.whatsappConsent !== 'opt_in') {
      return { 
        success: false, 
        error: 'Patient has not opted in for WhatsApp notifications or has no phone number' 
      };
    }

    const appointmentDate = format(new Date(appointment.startTime), "dd 'de' MMMM", { locale: ptBR });
...
Equipe FisioFlow`;
  ...
  async getMessageStatus(messageId: string): Promise<any> {
    return trackExternalAPICall('whatsapp', 'message_status', async () => {
      try {
        const response = await fetch(`${this.baseUrl}/${this.version}/${messageId}`, {
          headers: {
            'Authorization': `Bearer ${this.apiKey}`,
          },
        });
  ...
  private async processIncomingMessage(message: any): Promise<void> {
    ...
    if (['sair', 'stop', 'parar'].includes(messageBody)) {
      ...
      structuredLogger.info('Patient opted out of WhatsApp notifications', { from });
    }
  ...
  async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; error?: string }> {
    try {
      if (!this.apiKey || !this.phoneNumberId) {
        return { status: 'unhealthy', error: 'WhatsApp credentials not configured' };
      }

      // Simple health check - verify phone number
      const response = await fetch(`${this.baseUrl}/${this.version}/${this.phoneNumberId}`, {
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
        },
      });
